...
- Two frame-naming conventions share one atlas, and mixing them fails silently. Timed animations are keyed `name:index` (`purr:3`); the directional frames are whole keys with no index (`look:0,1`). `draw()` takes the first form, `drawFrame()` takes the second. Passing a look key to `draw()` builds `look:0,1:0`, misses, and leaves the previous frame on the canvas with no error — which broke eye-follow outright and froze him mid-blink after petting.
